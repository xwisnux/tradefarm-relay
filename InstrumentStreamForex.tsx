...
const FOREX_SYMBOLS = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD"]

const normalizeSymbol = (raw: string) => raw.replace(".b", "")

export default function InstrumentStreamForex(props: IProps) {
            ...
            switch (message.type) {
                case "Symbols":
                    const filtered = message.symbols.filter((s: string) =>
                        FOREX_SYMBOLS.includes(normalizeSymbol(s))
                    )
                    setSymbols(filtered)
...
InstrumentStreamForex.defaultProps = {
